...
public class AnvilChunkLoader implements IThreadedFileIO, IChunkLoader
{
    ...
    public Chunk loadChunk(World par1World, int par2, int par3) throws IOException
    {
        ...
        return func_75822_a(par1World, par2, par3, nbttagcompound);
    }
    ...
    protected void func_75824_a(ChunkCoordIntPair par1ChunkCoordIntPair, NBTTagCompound par2NBTTagCompound)
    {
        synchronized (field_75827_c)
        {
            if (field_75826_b.contains(par1ChunkCoordIntPair))
            {
                for (int i = 0; i < field_75828_a.size(); i++)
                {
                    if (((AnvilChunkLoaderPending)field_75828_a.get(i)).field_76548_a.equals(par1ChunkCoordIntPair))
                    {
                        field_75828_a.set(i, new AnvilChunkLoaderPending(par1ChunkCoordIntPair, par2NBTTagCompound));
                        return;
                    }
                }
            }

            field_75828_a.add(new AnvilChunkLoaderPending(par1ChunkCoordIntPair, par2NBTTagCompound));
            field_75826_b.add(par1ChunkCoordIntPair);
    ...
    {
        DataOutputStream dataoutputstream = RegionFileCache.getChunkOutputStream(chunkSaveLocation, par1AnvilChunkLoaderPending.field_76548_a.chunkXPos, par1AnvilChunkLoaderPending.field_76548_a.chunkZPos);
        CompressedStreamTools.write(par1AnvilChunkLoaderPending.field_76547_b, dataoutputstream);
    ...
    private void writeChunkToNBT(Chunk par1Chunk, World par2World, NBTTagCompound par3NBTTagCompound)
    {
        ...
        for (int k = 0; k < i; k++)
        {
            ...
            nbttagcompound.setByteArray("Blocks", extendedblockstorage.func_76658_g());
            ...
            nbttagcompound.setByteArray("Data", extendedblockstorage.func_76669_j().data);
