...
public class BlockBed extends BlockDirectional
{
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9)
    {
        ...
        if (!isBlockHeadOfBed(i))
        {
            int j = getDirection(i);
            par2 += footBlockToHeadBlockMap[j][0];
            par4 += footBlockToHeadBlockMap[j][1];
        ...
        if (!par1World.worldProvider.canRespawnHere())
        {
            ...
            par2 += footBlockToHeadBlockMap[k][0];
            par4 += footBlockToHeadBlockMap[k][1];
    ...
    public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
    {
        ...
        if (isBlockHeadOfBed(i))
        {
            if (par1World.getBlockId(par2 - footBlockToHeadBlockMap[j][0], par3, par4 - footBlockToHeadBlockMap[j][1]) != blockID)
            {
                par1World.setBlockWithNotify(par2, par3, par4, 0);
            }
        }
        else if (par1World.getBlockId(par2 + footBlockToHeadBlockMap[j][0], par3, par4 + footBlockToHeadBlockMap[j][1]) != blockID)
        {
            par1World.setBlockWithNotify(par2, par3, par4, 0);
    ...
    public static ChunkCoordinates getNearestEmptyChunkCoordinates(World par0World, int par1, int par2, int par3, int par4)
    {
        ...
        for (int k = 0; k <= 1; k++)
        {
            int l = par1 - footBlockToHeadBlockMap[j][0] * k - 1;
            int i1 = par3 - footBlockToHeadBlockMap[j][1] * k - 1;
            int j1 = l + 2;
            int k1 = i1 + 2;
            ...
            for (int l1 = l; l1 <= j1; l1++)
            {
                ...
                {
                    if (!par0World.func_72797_t(l1, par2 - 1, i2) || !par0World.isAirBlock(l1, par2, i2) || !par0World.isAirBlock(l1, par2 + 1, i2))
                    {
                        continue;
                    }
