...
public class EffectRenderer
{
        ...
        for (int j = 0; j < i; j++)
        {
            for (int k = 0; k < i; k++)
            {
                for (int l = 0; l < i; l++)
                {
                    ...
                    addEffect((new EntityDiggingFX(worldObj, d, d1, d2, d - (double)par1 - 0.5D, d1 - (double)par2 - 0.5D, d2 - (double)par3 - 0.5D, block, i1, par5)).func_70596_a(par1, par2, par3));
                }
            }
        }
    }
    ...
    public void addBlockHitEffects(int par1, int par2, int par3, int par4)
    {
        ...
        addEffect((new EntityDiggingFX(worldObj, d, d1, d2, 0.0D, 0.0D, 0.0D, block, par4, worldObj.getBlockMetadata(par1, par2, par3))).func_70596_a(par1, par2, par3).multiplyVelocity(0.2F).func_70541_f(0.6F));
    }
