...
public class GameSettings
{
    ...
    public String getKeyBinding(EnumOptions par1EnumOptions)
    {
        ...
        if (par1EnumOptions == EnumOptions.CHAT_VISIBILITY)
        {
            return (new StringBuilder()).append(s).append(func_74299_a(field_74369_af, chatVisibility)).toString();
        }
        ...
        try
        {
            ...
            for (String s = ""; (s = bufferedreader.readLine()) != null;)
            {
                try
                {
                    ...
                    {
                        chatVisibility = Integer.parseInt(as[1]);
                    }

                    if (as[0].equals("chatColors"))
                    {
                        chatColours = as[1].equals("true");
                    }

                    if (as[0].equals("chatLinks"))
                    {
                        chatLinks = as[1].equals("true");
                    }

                    if (as[0].equals("chatLinksPrompt"))
                    {
                        chatLinksPrompt = as[1].equals("true");
                    }

                    if (as[0].equals("chatOpacity"))
                    {
                        chatOpacity = parseFloat(as[1]);
                    }

                    if (as[0].equals("serverTextures"))
                    {
                        serverTextures = as[1].equals("true");
                    }

                    if (as[0].equals("snooperEnabled"))
                    {
                        snooperEnabled = as[1].equals("true");
                    }
    ...
    public void saveOptions()
    {
        try
        {
            ...
            printwriter.println((new StringBuilder()).append("chatVisibility:").append(chatVisibility).toString());
            printwriter.println((new StringBuilder()).append("chatColors:").append(chatColours).toString());
            printwriter.println((new StringBuilder()).append("chatLinks:").append(chatLinks).toString());
            printwriter.println((new StringBuilder()).append("chatLinksPrompt:").append(chatLinksPrompt).toString());
            printwriter.println((new StringBuilder()).append("chatOpacity:").append(chatOpacity).toString());
            printwriter.println((new StringBuilder()).append("serverTextures:").append(serverTextures).toString());
            printwriter.println((new StringBuilder()).append("snooperEnabled:").append(snooperEnabled).toString());
        ...
        {
            mc.thePlayer.sendQueue.addToSendQueue(new Packet204ClientInfo(language, renderDistance, chatVisibility, chatColours, difficulty));
        }
    }
