...
public class GuiControls extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        controlList.add(new GuiButton(200, width / 2 - 100, height / 6 + 168, stringtranslate.translateKey("Terminer")));
    ...
    public void drawScreen(int par1, int par2, float par3)
    {
        ...
        for (int j = 0; j < options.keyBindings.length; j++)
        {
            ...
            if (buttonId == j)
            {
                ((GuiButton)controlList.get(j)).displayString = "\247f> \247e??? \247f<";
            }
            else if (flag)
            {
                ((GuiButton)controlList.get(j)).displayString = (new StringBuilder()).append("\247c").append(options.getOptionDisplayString(j)).toString();
            }
            else
            {
                ((GuiButton)controlList.get(j)).displayString = options.getOptionDisplayString(j);
            }
