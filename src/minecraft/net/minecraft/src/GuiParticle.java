...
public class GuiParticle extends Gui
{
    ...
    public void draw(float par1)
    {
        ...
        for (Iterator iterator = particles.iterator(); iterator.hasNext(); drawTexturedModalRect(i, j, 40, 0, 8, 8))
        {
            Particle particle = (Particle)iterator.next();
            i = (int)((particle.prevPosX + (particle.posX - particle.prevPosX) * (double)par1) - 4D);
            j = (int)((particle.prevPosY + (particle.posY - particle.prevPosY) * (double)par1) - 4D);
