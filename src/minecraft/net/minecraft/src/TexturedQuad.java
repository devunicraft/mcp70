...
public class TexturedQuad
{
    ...
    public void draw(Tessellator par1Tessellator, float par2)
    {
        ...
        Vec3 vec3_1 = vertexPositions[1].vector3D.subtract(vertexPositions[2].vector3D);
        Vec3 vec3_2 = vec3_1.crossProduct(vec3).normalize();
        par1Tessellator.startDrawingQuads();

        if (invertNormal)
        {
            par1Tessellator.setNormal(-(float)vec3_2.xCoord, -(float)vec3_2.yCoord, -(float)vec3_2.zCoord);
        }
        else
        {
            par1Tessellator.setNormal((float)vec3_2.xCoord, (float)vec3_2.yCoord, (float)vec3_2.zCoord);
