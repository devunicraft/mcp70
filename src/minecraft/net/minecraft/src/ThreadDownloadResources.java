...
public class ThreadDownloadResources extends Thread
{
    ...
    private void loadResource(File par1File, String par2Str)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            if (file.isDirectory())
            {
                loadResource(file, (new StringBuilder()).append(par2Str).append(file.getName()).append("/").toString());
                continue;
            }

            try
            {
                mc.installResource((new StringBuilder()).append(par2Str).append(file.getName()).toString(), file);
            }
            catch (Exception exception)
            {
                System.out.println((new StringBuilder()).append("Failed to add ").append(par2Str).append(file.getName()).toString());
            }
        }
    }
